...
class OpenSSLConan(ConanFile):
    ...
    default_options["use_pic"] = True
    default_options["openssldir"] = None
    requires = ["zlib/1.2.11@bitwyre/stable"]
    exports_sources = "*"
    _env_build = None
    ...
    def _perlasm_scheme(self):
        # right now, we need to tweak this for iOS & Android only, as they
        # inherit from generic targets
    ...
    def _targets(self):
        ...
        return {
            ...
            "tvOS-*-*": "iphoneos-cross",
            # Android targets are very broken, see
    ...
    def _ancestor_target(self):
        ...
        if not ancestor:
            raise ConanInvalidConfiguration("unsupported configuration: %s %s %s, "
                                            ...
                                            "(list of configurations can be found by running './Configure --help')." % (self.settings.os,
                                            self.settings.arch,
                                            self.settings.compiler))
        return ancestor
    ...
    def _patch_configure(self):
        ...
        # use a more restricted regular expresion to prevent that Configure
        # script trying to do it again
        configure = os.path.join(self.build_folder, "Configure")
    ...
    def _patch_makefile_org(self):
        ...
        # its often easier to modify Configure and Makefile.org rather than
        # trying to add targets to the configure scripts
        def adjust_path(path):
            return path.replace("\\", "/") if tools.os_info.is_windows else path

        makefile_org = os.path.join(self.build_folder, "Makefile.org")
    ...
    def _configure_args(self):
        ...
        if self.settings.os == "Windows":
            ...
            args.append("-fPIC" if self.options.use_pic else "no-pic")
        ...
        if tools.os_info.is_windows:
            # clang-cl doesn't like backslashes in #define CFLAGS
            # (builldinf.h -> cversion.c)
        ...
        for option_name in self.options.values.fields:
            ...
            if activated and option_name not in ["use_pic", "openssldir", "capieng_dialog", "enable_capieng"]:
...
"""
        ...
        tools.save(os.path.join(self.build_folder, "Configurations", "20-conan.conf"), config)
    ...
    def _run_make(self, targets=None, makefile=None, parallel=True):
        ...
        if not self._use_nmake:
            # workaround for random error: size too large (archive member
    ...
    def _perl(self):
        if tools.os_info.is_windows and not self._win_bash:
            # enforce strawberry perl, otherwise wrong perl could be used (from
            # Git bash, MSYS, etc.)
    ...
    def _make_install(self):
        with tools.chdir(self.build_folder):
            ...
            # (https://github.com/openssl/openssl/issues/7653)
    ...
    def package(self):
        self.copy(src=self.build_folder, pattern="*LICENSE", dst="licenses")
        with tools.vcvars(self.settings) if self._use_nmake else tools.no_op():
            self._make_install()
        ...
        self._create_cmake_module_variables(os.path.join(self.package_folder, self._module_subfolder, self._module_file))
